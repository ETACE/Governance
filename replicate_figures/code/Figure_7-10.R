...
# load gamma low
load("data_gamma_0_001.Rdata")

rm(i, p1, p2, par1, par2, parameter1, parameter2, pathdata, pathfigs, probe, runs, parameter_list, data)

data_low <- DATA
rm(DATA)

# load gamma medium
load("data_gamma_0_01.Rdata")
...
# clean data
data_low <- data_low %>%
  filter(t == 2500 | t==5000 | t==10000 | t==50000)
...
par2 <- par1
timing <- c(2500, 5000, 10000, 50000)
...
for (i in 1:length(timing)){
  ...
  for(p1 in 1:length(par1))  {
    for(p2 in 1:length(par2))  {
      ...
      # low
      eval(parse(text=paste("temp = data_low[data_low$par1==par1[p1] & data_low$par2==par2[p2] & data_low$t==timing[i] ,]$",
                            variable,
                            sep="")))
      ...
      if (timing[i] %in% c(2500,5000) & count == 14){
      ...
      # medium
      eval(parse(text=paste("temp = data_medium[data_medium$par1==par1[p1] & data_medium$par2==par2[p2] & data_medium$t==timing[i] ,]$",
                            variable,
                            sep="")))
      ...
      DATA_MEDIUM = rbind(DATA_MEDIUM,
                          data.frame(r=1:runs,
                                     ...
                                     val=temp))
      # high
      eval(parse(text=paste("temp = data_high[data_high$par1==par1[p1] & data_high$par2==par2[p2] & data_high$t==timing[i] ,]$",
                            variable,
                            sep="")))
...
ggplot(TEMP_ALL_EQ, aes(x=Ownership.LTI, y=value, group=Manager.autonomy, col=Manager.autonomy)) +
